...
func validateBet(data engine.GameParams, txStore store.TransactionStore, game string) (engine.GameParams, rgse.RGSErr) {
	if data.Action != "base" {
		...
		stakeValues, _, _, _, err := parameterSelector.GetGameplayParameters(engine.Money{0, txStore.Amount.Currency}, txStore.BetLimitSettingCode, game, txStore.CompanyId)
		if err != nil {
			return data, err
		}
